#!/bin/sh
get_price() {
  PRICE=$(curl -s https://www.ing.cz/ing-podilove-fondy/kompletni-nabidka-ing-podilovych-fondu/$1/ | grep $2 | head -1 | awk '{$1=$1};1'| awk -F' ' '{ print $1 }' | tr ',' '.')
  echo "$1,$PRICE,$2"
}

get_price_ing() {
  PRICE=$(curl -s https://www.ing.cz/ing-podilove-fondy/nabidka-fondu/zakladni-nabidka-ing-podilovych-fondu-bez-poplatku/$1/ | grep $2 | grep -v '=' | head -1 | awk '{$1=$1};1' | awk -F' ' '{ print $1 }' | tr ',' '.')
  echo "$1,$PRICE,$2"
}
